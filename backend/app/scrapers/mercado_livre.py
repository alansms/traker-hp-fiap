...
try:
    ...
    # Inicializar o serviço Elasticsearch com tratamento de falhas
    try:
        es_service = ElasticsearchService()
        if es_service.client is None:
            logger = logging.getLogger(__name__)
            logger.warning("Elasticsearch não está disponível. Continuando sem armazenamento em Elasticsearch.")
    except Exception as e:
        logger = logging.getLogger(__name__)
        logger.error(f"Erro ao inicializar Elasticsearch: {e}")
        logger.warning("Continuando sem armazenamento em Elasticsearch.")
        es_service = None
except ImportError:
