...
(Versão para execução local - fora do Docker)
"""
import sys
import os
...
# Configurar URL do banco local antes de importar
...
def check_user_status():
    ...
    try:
        ...
        print("\n💡 Possíveis soluções:")
        print("1. Verifique se o PostgreSQL está rodando localmente na porta 5432")
        print("2. Execute o comando: brew services start postgresql")
        print("3. Ou execute via Docker: docker-compose up -d")
        return False
    finally:
        if db:
            db.close()
...
if __name__ == "__main__":
    ...
    print("🌐 Tentando conectar ao banco local (localhost:5432)")
    print("-" * 60)
    check_user_status()
