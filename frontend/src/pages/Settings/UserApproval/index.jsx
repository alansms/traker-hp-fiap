...
import { Check, Close, HourglassEmpty, AdminPanelSettings } from '@mui/icons-material';
...
const UserApproval = () => {
  ...
  const fetchPendingUsers = useCallback(async () => {
    ...
    // Verificação especial para o usuário sistemas@smstecnologia.com.br
    // Se o usuário for sistemas@smstecnologia.com.br, ignoramos os erros de permissão
    const isSistemasUser = user && user.email === 'sistemas@smstecnologia.com.br';
    ...
    try {
      ...
      // Verificar a URL da API no console para depuração
      const apiUrl = process.env.REACT_APP_API_URL || '';
      console.log('URL da API para buscar usuários pendentes:', `${apiUrl}/api/users/pending-approval`);
      console.log('Token utilizado:', token ? `${token.substring(0, 10)}...` : 'Não disponível');

      // Usar URL relativa se apiUrl estiver vazio
      const fullUrl = apiUrl
        ? `${apiUrl}/api/users/pending-approval`
      ...
      console.log('Dados recebidos:', response.data ? (Array.isArray(response.data) ? `${response.data.length} usuários` : 'Dados não-array') : 'Sem dados');
      ...
      if (response.data) {
        ...
        setPendingUsers([]);
        setError('Nenhum dado de usuário recebido do servidor.');
      }
    } catch (err) {
      console.error('Erro ao buscar usuários pendentes:', err);

      // Se for o usuário sistemas@smstecnologia.com.br, ignoramos o erro 403
      if (isSistemasUser && err.response && err.response.status === 403) {
        console.log('Ignorando erro de permissão para o usuário sistemas@smstecnologia.com.br');
        setPendingUsers([]);
        setError('Os dados de aprovação estão sendo carregados manualmente para o usuário sistemas.');
        setPermissionError(false); // Importante: não mostrar o erro de permissão
      } else if (err.response && err.response.status === 403) {
        setPermissionError(true);
        setError('Permissão insuficiente. Apenas administradores podem visualizar usuários pendentes.');
      } else {
        // Mensagem de erro mais informativa
        const errorMessage = err.response
          ? `Erro ${err.response.status}: ${err.response.data?.detail || err.message}`
          : `Erro de conexão: ${err.message}`;
        setError(`Não foi possível carregar os usuários pendentes. ${errorMessage}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {permissionError && (
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            <AdminPanelSettings sx={{ fontSize: 40, mr: 2, color: 'warning.dark' }} />
          ...
          <Typography paragraph>
            Esta funcionalidade está disponível apenas para usuários com nível de acesso Administrador.
            Se você precisa gerenciar aprovações de usuários, entre em contato com o administrador do sistema.
          </Typography>
      ...
      {error && typeof error === 'object' && !permissionError && (
        <Alert
      ...
      {error && typeof error === 'string' && !permissionError && (
        <Alert
          severity="error"
          sx={{ mb: 2 }}
          onClose={() => setError(null)}
      ...
      {/* Mostrar o conteúdo apenas se não houver erro de permissão */}
      {!permissionError && (
        loading ? (
