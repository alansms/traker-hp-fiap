...
const SearchPage = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await scrapingService.searchProducts(searchTerm);

      // Log detalhado da resposta completa para diagnóstico
      console.log('Resposta completa da API:', response);

      // Verifica a estrutura da resposta
      if (!response) {
        console.error('Resposta da API é undefined ou null');
        setSearchResults([]);
        return;
      }

      // Verifica se a resposta contém a propriedade results (formato esperado do backend)
      if (response.results) {
        ...
        if (results.length === 0 && response.count > 0) {
          console.warn(`Backend encontrou ${response.count} produtos, mas o frontend recebeu um array vazio`);
        }
      } else {
        // Se a resposta não contém a propriedade results, pode ser que a resposta já seja o array
        if (Array.isArray(response)) {
          console.log(`Recebidos ${response.length} produtos (resposta é um array) para "${searchTerm}"`);
